...
const AdminAllCars = () => {
  ...
  const user = useSelector((state) => state.userData?.user); // Get user ID from Redux state
  ...
  useEffect(() => {
    ...
    const fetchCars = async () => {
      try {
        const response = await fetch(`http://localhost:8080/api/admin/cars/${user.userId}`);
        if (!response.ok) {
